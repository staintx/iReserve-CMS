...
export default function BookingWizard() {
  ...
  // --- Fill in what we already know about the signed-in customer ---
  useEffect(() => {
    if (!user) return;
    setForm((prev) =>
      prev.customer_id && prev.contact_email
        ? prev
        : {
            ...prev,
      ...
      switch (stepId) {
        ...
        case "ReviewBooking": {
          if (!agreements.terms || !agreements.privacy) {
            errors.agreements =
              "Tick both boxes to confirm you've read the Terms and the Privacy Policy.";
          }
          break;
        }
      ...
      if (!message && errorCount > 0) {
        message =
          errorCount === 1
            ...
            : `${errorCount} details need your attention. They are highlighted above.`;
      }

      return { valid: errorCount === 0 && !message, errors, message };
    },
    [
      form,
      agreements,
          ...
          <StepPackageSelection
            ...
            onSelectPackage={(packageId) => {
              ...
              setForm((prev) => ({
                ...
                // The default option has to carry its price too. Without this,
                // a customer who accepted the pre-selected size saw it
                // highlighted while the estimate quietly used the business-wide
                // fallback instead of the price on the card.
