...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, X, Star } from "lucide-react";
            ...
            <div className="max-w-4xl">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="bg-black/40 backdrop-blur-md border border-white/20 text-white px-4 py-2 rounded-full text-sm font-medium flex items-center shadow-lg">
                  <span className="opacity-70 mr-1">Size:</span> {pkg.size || "Custom"}
                </div>
                ...
                <div className="bg-black/40 backdrop-blur-md border border-white/20 text-white px-4 py-2 rounded-full text-sm font-medium flex items-center shadow-lg">
                  {pkg.rating || "4.5"} <Star className="w-3.5 h-3.5 ml-1 fill-accent text-accent" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="bg-accent/5 border-accent/20">
                  <CardContent className="pt-6">
                    ...
                    <p className="text-sm text-muted-foreground pl-10">{cancellationPolicy}</p>
                  </CardContent>
                </Card>
                <Card className="bg-accent/5 border-accent/20">
                  <CardContent className="pt-6">
                    ...
                    <p className="text-sm text-muted-foreground pl-10">{bookingRequirements}</p>
                  </CardContent>
                </Card>
              </div>
            </section>
            ...
            <section>
              <div className="flex items-end justify-between mb-6 pb-4 border-b border-border">
                <h2 className="text-2xl font-serif font-bold text-foreground">Services & Inclusions</h2>
            ...
            <section>
              <div className="flex items-end justify-between mb-6 pb-4 border-b border-border">
                <h2 className="text-2xl font-serif font-bold text-foreground">Add Ons</h2>
                <span className="text-sm font-medium text-muted-foreground bg-muted px-3 py-1 rounded-full">
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <Card className="border-border shadow-lg overflow-hidden">
                <div className="bg-accent/10 p-6 border-b border-border">
                  <p className="text-sm font-bold text-accent uppercase tracking-wider mb-2">Package Price</p>
                  <h2 className="text-4xl font-serif font-bold text-foreground">₱{formatMoney(packagePrice)}</h2>
                </div>
                ...
                <CardContent className="p-6">
                  <div className="space-y-4 mb-8">
                    <div className="flex justify-between items-center py-2 border-b border-border border-dashed">
                      ...
                      <strong className="text-foreground">{pkg.size || "Custom"}</strong>
                    </div>
                    {pkg.max_guests && (
                      <div className="flex justify-between items-center py-2 border-b border-border border-dashed">
                        <span className="text-muted-foreground">Max Guests</span>
                        <strong className="text-foreground">{pkg.max_guests}</strong>
                      </div>
                    )}
                    {pkg.event_type && (
                      <div className="flex justify-between items-center py-2 border-b border-border border-dashed">
                        <span className="text-muted-foreground">Event Type</span>
                        <strong className="text-foreground">{pkg.event_type}</strong>
                      </div>
                    )}
                    <div className="flex justify-between items-center py-2 border-b border-border border-dashed">
                      <span className="text-muted-foreground">Duration</span>
                      <strong className="text-foreground">{pkg.duration || "4 hours"}</strong>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-border border-dashed">
                      <span className="text-muted-foreground">Availability</span>
                      <strong className={pkg.available ? "text-green-600" : "text-amber-600"}>
                        {pkg.available ? "Open" : "Limited"}
                      </strong>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <Button 
                      ...
                      onClick={() => navigate("/customer/book", { state: { eventType: derivedEventType, packageId: pkg._id, packagePrice: packagePrice, packageName: pkg.name } })}
                    >
                      Book Now
                    </Button>
                    <Button 
                      ...
                      onClick={() => navigate("/customer/quote", { state: { eventType: derivedEventType } })}
                    >
              ...
              <Card className="bg-primary text-primary-foreground border-none shadow-md">
                <CardContent className="p-6">
                  ...
                  <p className="text-primary-foreground/80 text-sm mb-6 leading-relaxed">
                    Send us your event details and we will tailor the package to match your theme and guests.
                  </p>
                  <Button 
                    variant="secondary"
                    className="w-full"
        ...
        <section className="bg-accent/10 border-t border-accent/20 mt-12 py-16">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <p className="text-muted-foreground text-lg mb-8 max-w-2xl mx-auto">
              We will confirm availability and secure your date within 24 hours. Start planning your unforgettable event today.
            </p>
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <button 
            ...
            onClick={(e) => { 
              e.stopPropagation(); 
              setLightboxIndex((lightboxIndex - 1 + pkg.gallery.length) % pkg.gallery.length); 
            }}
          >
