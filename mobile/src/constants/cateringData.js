/**
 * cateringData.js
 * Curated high-resolution culinary photography and event portfolio data
 * for Caezelle's Food, Catering & Services.
 */
...
export const DISH_IMAGE_REGISTRY = {
  ...
  "lechon kawali":
    "https://images.unsplash.com/photo-1603360946369-dc9bb6258143?auto=format&fit=crop&w=600&q=80",
  "sweet and sour pork":
...
/**
 * Returns a high-resolution food photo URL for a menu item
 */
export const resolveDishImage = (dish) => {
  if (dish?.image_url && dish.image_url.trim() !== "") {
    return dish.image_url;
  }
  const name = String(dish?.name || "").toLowerCase().trim();
  for (const [key, url] of Object.entries(DISH_IMAGE_REGISTRY)) {
...
/**
 * Resolves the primary cover photo for a package (matches website behavior)
 */
export const resolvePackageCover = (pkg) => {
  if (pkg?.image_url && typeof pkg.image_url === "string" && pkg.image_url.trim() !== "") {
    return pkg.image_url.trim();
  }
  if (Array.isArray(pkg?.gallery) && pkg.gallery.length > 0 && typeof pkg.gallery[0] === "string" && pkg.gallery[0].trim() !== "") {
    return pkg.gallery[0].trim();
  }
  return null;
};

/**
 * Resolves 3 preview photos for a package card if available
 */
export const resolvePackagePreviewDishes = (pkg) => {
  const photos = [];
  if (pkg?.image_url) {
    photos.push({ name: pkg.name || "Main Setup", image: pkg.image_url });
  }
  if (Array.isArray(pkg?.gallery)) {
    pkg.gallery.forEach((url, i) => {
      if (url && photos.length < 3) {
        photos.push({ name: `${pkg.name || "Setup"} ${i + 1}`, image: url });
      }
    });
  }
  if (Array.isArray(pkg?.menu_items) && pkg.menu_items.length > 0) {
    pkg.menu_items.forEach((item) => {
      if (photos.length < 3) {
        ...
        if (name) {
          photos.push({
            name,
            image: resolveDishImage(typeof item === "object" ? item : { name }),
          });
        }
      }
    });
...
/**
 * Curated Event Setup Portfolio Gallery (Fallback empty so live DB data is prioritized)
 */
export const CURATED_GALLERY_ITEMS = [];
