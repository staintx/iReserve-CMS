...
export default function InboxHub({ basePath = "/admin/messages" }) {
  ...
  return (
    <div className="h-full min-h-0 flex-1 w-full bg-white flex flex-col md:flex-row font-sans antialiased overflow-hidden">
      ...
      <div className={cn(
        "w-full md:w-80 lg:w-88 border-r border-slate-200 flex flex-col min-h-0 bg-white shrink-0 transition-all",
        ...
        <ScrollArea className="flex-1 divide-y divide-slate-100">
          ...
          {filteredThreads.map((thread) => {
            ...
            return (
              ...
              >
                <div className="w-8 h-8 rounded-md bg-[#2C4B8A]/10 text-[#2C4B8A] border border-[#2C4B8A]/20 font-bold flex items-center justify-center text-xs font-mono shrink-0 mt-0.5 shadow-2xs">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-1">
                    <span className="text-[10px] py-0.5 px-1.5 font-mono rounded-md bg-slate-100 text-slate-600 border border-slate-200/80 font-bold">
                      {badge.text}
                    </span>
                    {isUnread && (
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-white">
          ...
          <div className="h-14 px-5 border-b border-slate-200 bg-white flex items-center justify-between gap-3 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="w-8 h-8 rounded-md bg-[#2C4B8A]/10 text-[#2C4B8A] border border-[#2C4B8A]/20 font-bold flex items-center justify-center text-xs font-mono shrink-0 shadow-2xs">
          ...
          {isCustomerRole && activeBooking && (
            <div className="bg-slate-50 border-b border-slate-200/80 px-5 py-2 flex items-center justify-between gap-2 shrink-0">
              <div className="flex items-center gap-2 text-xs min-w-0">
                ...
                <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 rounded-md bg-white border border-slate-200 text-slate-700 font-bold shrink-0">
                ...
                <div key={gIdx} className="space-y-4">
                  ...
                  {group.items.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        {!isMe && (
                          <div className="w-7 h-7 rounded-md bg-slate-100 text-slate-600 font-bold flex items-center justify-center text-xs font-mono shrink-0 border border-slate-200/60 mb-1 shadow-2xs">
                        ...
                        <div className={cn("flex flex-col max-w-[78%]", isMe ? "items-end" : "items-start")}>
                              ...
                              <div className="mt-2.5 space-y-1 border-t border-slate-200/40 pt-2">
                                {msg.attachments.map((att, aIdx) => (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="flex items-center gap-2 p-2 rounded-md bg-white/20 hover:bg-white/40 transition text-xs font-medium underline truncate"
                                  >
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-slate-400 mt-1 px-0.5 font-mono">
          ...
          {!isCustomerRole && (
            <div className="px-4 py-2 border-t border-slate-200 bg-slate-50/60 flex items-center gap-2 overflow-x-auto [scrollbar-width:none] shrink-0">
              <Button
                size="xs"
                variant="outline"
                onClick={async () => {
              ...
              {QUICK_REPLIES.map((reply, idx) => (
                <button
                  ...
                  disabled={isSending}
                  className="text-xs bg-white hover:bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md whitespace-nowrap transition-colors border border-slate-200 cursor-pointer shadow-2xs"
                >
          ...
          {aiDraft && (
            <div className="px-4 py-3 bg-amber-50/80 border-t border-amber-200 flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-2 shrink-0">
              <div className="flex items-center justify-between">
          ...
          {showAttachmentInput && (
            <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 flex items-center gap-2 shrink-0">
          ...
          <div className="p-3.5 sm:p-4 bg-white border-t border-slate-200 shrink-0">
            <form
              ...
              className="max-w-3xl mx-auto rounded-md border border-slate-200 bg-white p-2.5 sm:p-3 shadow-2xs focus-within:border-slate-400 transition-all space-y-2"
            >
              <textarea
                className="w-full resize-none bg-transparent text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 outline-none min-h-11 max-h-28 leading-relaxed"
      ...
      {activeId && showDetailsPane && (
        <div className="hidden lg:flex w-80 border-l border-slate-200 bg-slate-50/40 flex-col p-4 space-y-4 shrink-0 overflow-y-auto min-h-0">
          ...
          {activeCustomer && (
            <div className="p-4 bg-white rounded-md border border-slate-200 shadow-2xs space-y-2">
              ...
              {activeCustomer.phone && (
                <p className="text-xs text-slate-600 pt-1.5 border-t border-slate-100 font-mono">
                  📞 {activeCustomer.phone}
                </p>
              )}
            </div>
            ...
            <div className="p-4 bg-white rounded-md border border-slate-200 shadow-2xs space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-100">
                <div>
                  ...
                  <span className="font-semibold text-slate-900 font-mono">{activeBooking.guests || 0} pax</span>
                </div>
                <div>
              ...
              <Button
                ...
                className="w-full text-xs h-8 gap-1.5 mt-1 rounded-md border-slate-200 font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer shadow-2xs"
              >
